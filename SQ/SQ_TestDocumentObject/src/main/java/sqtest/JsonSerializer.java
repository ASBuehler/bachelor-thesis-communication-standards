...
public class JsonSerializer implements Serializer {
    ...
    public <T> byte[] serialize(T object) throws Exception {
        try {
            ...
            // Vereinfachte, aber aussagekräftige Fehlermeldung
            String detailedMessage = String.format(
                "Jackson Fehler bei der Serialisierung von Typ: %s. Ursprüngliche Nachricht: %s",
                e.getType().getRawClass().getName(), // Gibt uns den genauen Klassennamen, der Probleme macht
                e.getOriginalMessage()
            );
            // Wir werfen eine neue Exception, die die Details enthält.
