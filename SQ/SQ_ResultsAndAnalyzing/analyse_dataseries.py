...
# --- 0. SETUP ---
output_dir = 'diagramme_dataseries'
...
# Filtere nur die DataSeries Testfälle
df_dataseries_all = df[df['Testfall'].str.contains("DataSeries")].copy()

# Berechne Gesamtzeit in Mikrosekunden
df_dataseries_all['Gesamtzeit_us'] = (df_dataseries_all['Serialisierungszeit_ns'] + df_dataseries_all['Deserialisierungszeit_ns']) / 1000
...
# Finde alle einzigartigen DataSeries Testfälle
unique_test_cases = df_dataseries_all['Testfall'].unique()
print(f"Folgende DataSeries-Testfälle wurden gefunden: {list(unique_test_cases)}")
...
for test_case in unique_test_cases:
    ...
    df_current = df_dataseries_all[df_dataseries_all['Testfall'] == test_case].copy()
    
    # Erstelle einen sauberen Dateinamen-Präfix
    file_prefix = test_case.lower().replace(' ', '_').replace('(', '').replace(')', '').replace(',', '').replace('ä', 'ae')

    # --- Diagramm A: Datengröße für diesen spezifischen Testfall ---
    plt.figure(figsize=(16, 9))
    sns.barplot(data=df_current.sort_values('Groesse_Bytes'), x='Datenformat', y='Groesse_Bytes', palette='viridis')
    ...
    plt.figure(figsize=(16, 9))
    sns.barplot(data=df_current.sort_values('Gesamtzeit_us'), x='Datenformat', y='Gesamtzeit_us', palette='viridis')
    ...
    plt.figure(figsize=(16, 9))
    sns.barplot(data=df_zeit_detail, x='Datenformat', y='Zeit_us', hue='Operation', palette='plasma')
...
print(f"\nAnalyse der DataSeries-Tests erfolgreich abgeschlossen. Alle Diagramme im Ordner '{output_dir}' gespeichert.")