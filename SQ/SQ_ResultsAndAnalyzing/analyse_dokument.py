...
# Filtere nur die Dokument Testfälle
df_dokument_all = df[df['Testfall'].str.contains("Dokument")].copy()

# Berechne Gesamtzeit in Mikrosekunden
df_dokument_all['Gesamtzeit_us'] = (df_dokument_all['Serialisierungszeit_ns'] + df_dokument_all['Deserialisierungszeit_ns']) / 1000
...
# Finde alle einzigartigen Dokument Testfälle
unique_test_cases = df_dokument_all['Testfall'].unique()
print(f"Folgende Dokument-Testfälle wurden gefunden: {list(unique_test_cases)}")
...
for test_case in unique_test_cases:
    ...
    df_current = df_dokument_all[df_dokument_all['Testfall'] == test_case].copy()
    ...
    sns.barplot(data=df_current.sort_values('Groesse_Bytes'), x='Datenformat', y='Groesse_Bytes', palette='cubehelix')
    ...
    plt.figure(figsize=(16, 9))
    sns.barplot(data=df_current.sort_values('Gesamtzeit_us'), x='Datenformat', y='Gesamtzeit_us', palette='cubehelix')
    ...
    plt.figure(figsize=(16, 9))
    sns.barplot(data=df_zeit_detail, x='Datenformat', y='Zeit_us', hue='Operation', palette='rocket')
...
print(f"\nAnalyse der Dokument-Tests erfolgreich abgeschlossen. Alle Diagramme im Ordner '{output_dir}' gespeichert.")